...
namespace myengine
{
	...
	void Mesh::loadAsset(std::string _assetFolderPath, std::string _fileName)
	{
		// Find file
		std::ifstream inputFile(_assetFolderPath + _fileName);
		...
		if (inputFile.is_open())
		{
			...
			while (std::getline(inputFile, currentLine))
			{
				...
				else if (!currentLine.substr(0, 2).compare(0, 1, "f"))
				{
					...
					{
						Debugger::printWarning("This OBJ loader only works with triangles but a quad has been detected. Please triangulate your mesh.");
						inputFile.close();
						return;
					}
		...
		{
			Debugger::printWarning("WARNING: File not found: " + _fileName);
		}
	}
